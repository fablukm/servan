...
- [ ] S-04 `servan status`: fenced sections, graceful "bd not installed" (exit 2 + install hint), flag-compat probe for bd status names
- [ ] S-05 CLI polish: `--version`, `--config-dir`, exit-code table honored, `servan sync --check` (diff-only, no write; exit 3 if drift)
